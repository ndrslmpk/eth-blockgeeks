...
contract Geekt {
    ...
    function removeUser(address badUser)
        public
        onlyAdmin
    ...
    function removeImage(bytes32 badImage)
        public
        onlyAdmin
        returns (bool success)
    {
    ...
    function registerNewUser(
        ...
        if (
            bytes(Users[msg.sender].handle).length == 0 && //checks whether or not the User already has an address
            bytes(handle).length != 0 //checks that the handle value is not null
        ) {
            // Creates new user
            ...
            usersByAddress.push(thisNewAddress); // PUSH() adds an entry for this user to the user 'whitepages'
            return true;
        } else {
    ...
    // -------------------------------------------------------
    // --> are free to call and read-only functions / not state changing
